...
public class ThundersUnderTheSea extends Attack {
    ...
    public boolean thunderRain(Board tableroEnemigo) {
        ...
        for (int i = 0; i < 100; i++) { //100 casillas
            ...
            if (tableroEnemigo.getTile(pos[0], pos[1]).daño(damage, "Thunder Rain")) {
                ataquesExitosos++;
            }
        }
    ...
    public boolean poseidonThunders(Board tableroEnemigo) {
            ...
            for (int r = pos[0] - radio; r <= pos[0] + radio; r++) {
                for (int c = pos[1] - radio; c <= pos[1] + radio; c++) {
                    if (tableroEnemigo.inBounds(r, c)) {
                        ...
                        if (tableroEnemigo.getTile(r, c).daño(vidaActual, "Poseidon Thunder")) {
                            ataquesExitosos++;
                        }
                    }
                }
    ...
    public boolean elAttack(Board tableroEnemigo) {
        ...
        for (int i = 0; i < numAnguilas; i++) {
            ...
            for (int j = 0; j < descargas; j++) {
                ...
                if (tableroEnemigo.getTile(pos[0], pos[1]).daño(damage, "El Attack")) {
                    ataquesExitosos++;
                }
            }
        }
