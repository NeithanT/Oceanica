...
public class CommandAttack extends Command implements Serializable {
    ...
    private CommandResult ejecutarFishTelepathy(FishTelepathy ataque, String jugadorObjetivo, String metodo, Board tablero, CommandContext ctx) {
        ...
        switch (metodo) {
            case "CARDUMEN":
                exitoso = ataque.cardumen(tablero);
                detalles = "Cardumen de peces lanzado";
                break;
            case "SHARK":
                exitoso = ataque.sharkAttack(tablero);
                detalles = "Ataque de tiburones desde las esquinas";
                break;
            case "PULP":
                exitoso = ataque.pulp(tablero);
        ...
        AttackStatistics.getInstance().registrarAtaque(ctx.player().getName(), "FISH_TELEPATHY - " + metodo, exitoso);

        //ctx.console().refreshBoard();
        String resultado = exitoso ? "exitosamente" : "pero no causó daño";
        return CommandResult.ok("Ataque " + metodo + " ejecutado " + resultado + ".");
    }
    ...
    private CommandResult ejecutarThunders(ThundersUnderTheSea ataque, String jugadorObjetivo, String metodo, Board tablero, CommandContext ctx) {
        ...
        switch (metodo) {
            case "THUNDERRAIN":
            case "RAIN":
                exitoso = ataque.thunderRain(tablero);
                detalles = "Lluvia de truenos cayendo";
                break;
            case "POSEIDON":
                exitoso = ataque.poseidonThunders(tablero);
                detalles = "Truenos de Poseidón cayendo";
                break;
            case "ELATTACK":
                exitoso = ataque.elAttack(tablero);
        ...
        //registrar en estadísticas
        AttackStatistics.getInstance().registrarAtaque(ctx.player().getName(), "THUNDERS_UNDER_THE_SEA - " + metodo, exitoso);
    ...
    private CommandResult ejecutarKraken(ReleaseTheKraken ataque, String jugadorObjetivo, String metodo, String[] args, Board tablero, CommandContext ctx) {
        ...
        switch (metodo) {
           ...
           case "TENTACULOS":
               ...
               try {
                   ...
                   exitoso = ataque.tentaculos(tablero, fila1, col1, fila2, col2, fila3, col3);
                   detalles = "Tres tentáculos del Kraken lanzados";
            ...
            case "ALIENTO":
                ...
                try {
                    ...
                    exitoso = ataque.krakenBreath(tablero, fila, col, direccion);
            ...
            case "RELEASE":
                if (args.length >= 3) {
                    ...
                    try {
                        ...
                        int col = Integer.parseInt(args[2]) - 1;
                        exitoso = ataque.releaseTheKraken(tablero, fila, col);
                        detalles = "¡Kraken liberado en posición específica!";
                    } catch (NumberFormatException e) {
                        return CommandResult.fail("Fila y cola deben ser números.");
                    }
                } else {
                    ...
                    exitoso = ataque.releaseTheKraken(tablero, pos[0], pos[1]);
                    detalles = "¡Kraken liberado en posición aleatoria!";
                }
                break;
            default:
        ...
        //registrar en estadísticas
        AttackStatistics.getInstance().registrarAtaque(ctx.player().getName(), "RELEASE_THE_KRAKEN - " + metodo, exitoso);
    ...
    private CommandResult ejecutarWavesControl(WavesControl ataque, String jugadorObjetivo, String metodo, Board tablero, CommandContext ctx) {
        ...
        switch (metodo) {
            case "SWIRL":
            case "SWIRLRAISING":
                exitoso = ataque.swirlRaising(tablero);
                detalles = "Remolino creado y destruyendo casillas";
                break;
            case "GARBAGE":
            case "SENDGARBAGE":
                exitoso = ataque.sendHumanGarbage(tablero);
                detalles = "Basura humana enviada desde remolino";
                break;
            case "RADIOACTIVE":
            case "RADIOACTIVERUSH":
                exitoso = ataque.radioactiveRush(tablero);
        ...
        //registrar en estadísticas
        AttackStatistics.getInstance().registrarAtaque(ctx.player().getName(), "WAVES_CONTROL - " + metodo, exitoso);
    ...
    private CommandResult ejecutarUnderseaVolcanoes(UnderseaVolcanoes ataque, String jugadorObjetivo, String metodo, Board tablero, CommandContext ctx) {
        ...
        //registrar en estadísticas
        AttackStatistics.getInstance().registrarAtaque(ctx.player().getName(), "UNDERSEA_VOLCANOES - " + metodo, exitoso);
    ...
    private CommandResult ejecutarTheTrident(TheTrident ataque, String jugadorObjetivo, String metodo, String[] args, Board tablero, CommandContext ctx) {
        ...
        switch (metodo) {
            ...
            case "3LINES":
                ...
                try {
                    ...
                    exitoso = ataque.threeLines(tablero, puntos);
            ...
            case "3NUMBERS":
                ...
                try {
                    ...
                    exitoso = ataque.threeNumbers(tablero, numerosTrident, numerosAtacado);
                    detalles = exitoso ? "Juego de números ejecutado - ¡Acertaste!" : "Juego de números ejecutado - No acertaste";
                } catch (NumberFormatException e) {
        ...
        //registrar en estadísticas
        AttackStatistics.getInstance().registrarAtaque(ctx.player().getName(), "THE_TRIDENT - " + metodo, exitoso);
